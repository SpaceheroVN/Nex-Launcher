...
class HelpDialog(QDialog):
    def __init__(self, help_text, parent=None):
        ...
        # All styling is now handled by the main stylesheet in style.py via #helpContainer
...
class ProgressDialog(QDialog):
    def __init__(self, max_value, parent=None, lang='EN'):
        ...
        self.close_button.setObjectName("acceptButton") # Changed to use global style
    ...
    def setup_items(self, items):
        for i, item in enumerate(items):
            ...
            status_label.setProperty("status", "waiting") # Use dynamic property
    ...
    def update_item_status(self, item_name, status, details):
        ...
        status_label.setProperty("status", status) # Set property for styling
        
        # Update text based on language
        status_text_map = {
            "downloading": self.t.get('progress_status_downloading', "Downloading..."),
        ...
        if status == "failed":
            status_label.setToolTip(details)

        # Re-polish to apply new style from stylesheet
...
class AboutDialog(QDialog):
    def __init__(self, parent=None, lang='EN'):
        ...
        version_label = QLabel(t.get('about_version', "Version 1.2"))
        author_label = QLabel(t.get('about_author', "Author: SpaceheroVN"))
        github_link = QLabel(t.get('about_github', '<a href="https://github.com/SpaceheroVN/NEX/releases/">Source Code on GitHub</a>'))
...
class SettingsDialog(QDialog):
    ...
    def show_about_dialog(self):
        AboutDialog(self, lang=self.lang).exec()
