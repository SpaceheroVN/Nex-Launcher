...
from PyQt6.QtGui import QIcon, QKeySequence, QShortcut, QAction, QCloseEvent, QPainter
from PyQt6.QtWidgets import (QApplication, QCheckBox, QHBoxLayout, QLabel,
                             QMainWindow, QMessageBox, QPushButton, QScrollArea,
                             QVBoxLayout, QWidget, QLineEdit, QSystemTrayIcon, QMenu)
...
class UninstallerWindow(QMainWindow):
    def __init__(self, app_instance, launcher_instance, language, theme):
        super().__init__()
    ...
    def _setup_ui(self):
        ...
        self.list_layout.setContentsMargins(0, 5, 0, 5); self.list_layout.setSpacing(0); self.list_layout.addStretch()
    ...
    def _create_list_header(self):
        ...
        header_layout.setContentsMargins(0, 0, 10, 0); header_layout.setSpacing(10)
        ...
        for key, (text, stretch) in header_data.items():
            ...
            btn.clicked.connect(partial(self.on_header_clicked, key))
            header_layout.addWidget(btn, stretch)
    ...
    def start_scan(self):
        ...
        self.scanner = AppScannerWorker()
        self.scanner.finished.connect(self.populate_app_list)
        self.scanner.start()

    def populate_app_list(self, apps):
        self.loading_label.deleteLater(); self.all_apps = apps; self.search_input.clear()
        self.set_controls_enabled(True)
        if not apps:
            no_apps_label = QLabel(self.t['no_apps_found_body']); no_apps_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
            self.list_layout.insertWidget(0, no_apps_label)
        else: self.sort_and_redisplay_apps()
    ...
    def update_header_indicators(self):
        for key, btn in self.header_buttons.items():
            ...
            if key == self.sort_key: btn.setText(f"{text} {'▲' if self.sort_order == Qt.SortOrder.AscendingOrder else '▼'}")
    ...
    def sort_and_redisplay_apps(self):
        ...
        def sort_key_func(app):
            ...
            if self.sort_key == "size_kb": return val or -1
            if self.sort_key == "install_date": return val or "0"
            return str(val or "").lower()
        self.all_apps.sort(key=sort_key_func, reverse=reverse)
        self.clear_list()
        for i, app_data in enumerate(self.all_apps):
            row_widget = create_app_row_widget(app_data, self.fallback_icon)
    ...
    def on_checkbox_toggled(self, index, state):
        ...
        if modifiers == Qt.KeyboardModifier.ShiftModifier and self.last_clicked_index != -1:
            ...
            # Với Ctrl, không làm gì cả, chỉ để checkbox tự thay đổi trạng thái
            pass
        else:
            # Click chuột bình thường, bỏ chọn tất cả các mục khác
        ...
        # Luôn cập nhật last_clicked_index cho lần Shift+Click tiếp theo
    ...
    def get_all_row_widgets(self, visible_only=False):
        ...
        for i in range(self.list_layout.count() - 1):
            ...
            if widget and widget.objectName() == "rowContainer" and not (visible_only and not widget.isVisible()):
    ...
    def set_controls_enabled(self, enabled):
        ...
        for btn in self.header_buttons.values(): btn.setEnabled(enabled)
