...
TRANSLATIONS = {
    'Installer_Help': {
        'EN': {
            ...
            'multi_thread_help': "Allows multiple apps to be installed simultaneously. It's faster but may cause issues with certain installers (like MSI). Disable for sequential, safer installation.",
            'auto_select_help': "After adding a new software item, it will be automatically checked in the installation list.",
            'auto_minimize_help': "Instead of showing the main window after an install, the app will remain in the system tray. The main window will also minimize to tray on close.",
            'detailed_categories_help': "Organizes apps into specific categories ('Web Browsers', etc.) on the 'Apps' tab. If disabled, all apps are shown in a single list.",
            'notifications_help': "Opens a dialog to configure which notifications to show, including the installation progress dialog.",
        },
        'VN': {
            'settings_help_title': "Trợ giúp Cài đặt",
            'multi_thread_help': "Cho phép cài đặt nhiều ứng dụng cùng lúc, giúp tốc độ nhanh hơn nhưng có thể gây lỗi với một số trình cài đặt (như MSI). Tắt để cài đặt tuần tự, an toàn hơn.",
            'auto_select_help': "Sau khi thêm một phần mềm mới, nó sẽ được tự động đánh dấu chọn trong danh sách cài đặt.",
            'auto_minimize_help': "Thay vì hiện cửa sổ chính sau khi cài đặt xong, ứng dụng sẽ giữ ở khay hệ thống. Cửa sổ chính cũng sẽ thu về khay khi đóng.",
            'detailed_categories_help': "Sắp xếp các ứng dụng vào các danh mục cụ thể (VD: 'Trình duyệt Web') trong tab 'Ứng dụng'. Nếu tắt, tất cả ứng dụng sẽ nằm trong một danh sách duy nhất.",
            'notifications_help': "Mở hộp thoại để tùy chỉnh các loại thông báo sẽ hiển thị, bao gồm cả hộp thoại tiến trình cài đặt.",
        }
    },
...
<p><i><b>Tip:</b> Always check the software's official documentation for the most accurate arguments.</i></p>
            ...
            'progress_status_already_installed': "Already installed!",
            'close_btn': "Close",
            ...
            'multi_thread_install_check': "Multi-thread installation",
            'notifications_label': "Notifications:",
            'edit_btn': "Edit...",
            'notification_settings_title': "Notification Options",
            'notify_on_hide_check': "Notify when minimized to tray",
            'notify_on_complete_check': "Notify when installation is complete",
        },
        'VN': {
...
<p><i><b>Mẹo:</b> Luôn kiểm tra tài liệu chính thức của phần mềm để có tham số chính xác nhất.</i></p>
            ...
            'progress_status_already_installed': "Đã được cài đặt trước đó!",
            'close_btn': "Đóng",
            ...
            'multi_thread_install_check': "Cài đặt đa luồng",
            'notifications_label': "Thông báo:",
            'edit_btn': "Chỉnh sửa...",
            'notification_settings_title': "Tùy chọn Thông báo",
            'notify_on_hide_check': "Thông báo khi thu nhỏ về khay",
            'notify_on_complete_check': "Thông báo khi cài đặt hoàn tất",
        }
    }
}
...
def get_installer_style(theme_name: str) -> str:
    ...
    dark_c = get_theme_colors('Dark')
    progress_bar_style = f"""
        QProgressBar {{
            border: 1px solid {dark_c['border_color']};
            border-radius: 5px;
            text-align: center;
            color: {dark_c['text_color']};
            background-color: {dark_c['input_bg']};
        }}
        QProgressBar::chunk {{
            background-color: {dark_c['primary_color']};
            border-radius: 5px;
        }}
    """

    return f"""
        {base_style} {scrollbar_style} {fix_style} {dark_specific} {progress_bar_style}
        ...
        QPushButton#removeButton {{ background-color: #D20103; }}
        QPushButton#removeButton:hover {{ background-color: #b80002; }}
        ...
        QPushButton#helpButton {{
            ...
            border-radius: 12px;
            padding: 0;
            background-color: {c['help_button_bg']};
            color: #0BFFFF;
            border: 1px solid transparent;
        }}
        QPushButton#helpButton:hover {{
            background-color: {c['help_button_hover_bg']};
            color: {c['help_button_hover_text']};
            border-color: {c['primary_color']};
        }}
        ...
        QLabel[status="installed"] {{ color: #FF1493; font-weight: bold; }}
    """