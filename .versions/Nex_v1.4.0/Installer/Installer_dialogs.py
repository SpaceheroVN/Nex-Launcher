...
class NotificationSettingsDialog(QDialog):
    def __init__(self, current_settings, parent=None, lang='EN'):
        ...
        self.setWindowTitle(t['notification_settings_title'])
        self.setMinimumWidth(350)
        self.setModal(True)
        
        layout = QVBoxLayout(self)
        
        self.progress_check = QCheckBox(t['show_progress_check'])
        self.progress_check.setChecked(current_settings.get("show_progress", True))
        layout.addWidget(self.progress_check)

        self.hide_check = QCheckBox(t.get('notify_on_hide_check', 'Notify when minimized to tray'))
        self.hide_check.setChecked(current_settings.get("on_hide", True))
        layout.addWidget(self.hide_check)

        self.complete_check = QCheckBox(t.get('notify_on_complete_check', 'Notify when installation is complete'))
        self.complete_check.setChecked(current_settings.get("on_complete", True))
        layout.addWidget(self.complete_check)
    ...
    def get_settings(self):
        return {
            ...
            "on_hide": self.hide_check.isChecked(),
            "on_complete": self.complete_check.isChecked()
        }
...
class ProgressDialog(QDialog):
    def __init__(self, max_value, parent=None, lang='EN', always_on_top=False):
        super().__init__(parent)
        self.lang = lang
    ...
    def update_item_status(self, item_name, status, details):
        ...
        status_text_map = {
            ...
            "installed": self.t.get('progress_status_already_installed', "Already installed!"),
        }
...
class SettingsDialog(QDialog):
    def __init__(self, current_settings, parent=None, lang='EN'):
        ...
        self.multi_thread_check = QCheckBox(t['multi_thread_install_check'])
        self.multi_thread_check.setChecked(self.settings.get("multi_thread_install", False))
        layout.addWidget(self.multi_thread_check)
        
        separator1 = QFrame()
        separator1.setFrameShape(QFrame.Shape.HLine); separator1.setFrameShadow(QFrame.Shadow.Sunken)
        layout.addWidget(separator1)

        self.autoselect_check = QCheckBox(t['auto_select_check'])
        self.autoselect_check.setChecked(self.settings.get("auto_select_add", False))
        layout.addWidget(self.autoselect_check)

        self.minimize_check = QCheckBox(t['auto_minimize_tray_check'])
        self.minimize_check.setChecked(self.settings.get("auto_minimize_tray", True))
        layout.addWidget(self.minimize_check)
        
        self.detailed_categories_check = QCheckBox(t['detailed_categories_check'])
        self.detailed_categories_check.setChecked(self.settings.get("use_detailed_categories", False))
        ...
        self.notifications_label = QLabel(t['notifications_label'])
        self.edit_notifications_btn = QPushButton(t['edit_btn'])
        self.edit_notifications_btn.clicked.connect(self.open_notification_settings)
        
        notification_layout.addWidget(self.notifications_label)
        notification_layout.addStretch()
        notification_layout.addWidget(self.edit_notifications_btn)
    ...
    def open_notification_settings(self):
        notification_data = {
            ...
            "on_hide": self.settings.get("notifications", {}).get("on_hide", True),
            "on_complete": self.settings.get("notifications", {}).get("on_complete", True)
        }
        dialog = NotificationSettingsDialog(notification_data, self, self.lang)
        if dialog.exec():
            new_data = dialog.get_settings()
            self.settings["show_progress"] = new_data["show_progress"]
            if "notifications" not in self.settings:
                self.settings["notifications"] = {}
            self.settings["notifications"]["on_hide"] = new_data["on_hide"]
            self.settings["notifications"]["on_complete"] = new_data["on_complete"]

    def get_settings(self):
        self.settings['multi_thread_install'] = self.multi_thread_check.isChecked()
        self.settings['auto_select_add'] = self.autoselect_check.isChecked()
        self.settings['auto_minimize_tray'] = self.minimize_check.isChecked()
        self.settings['use_detailed_categories'] = self.detailed_categories_check.isChecked()
        return self.settings

    def toggle_help_dialog(self, checked):
        if checked:
            if not self.help_dialog:
                t_help = TRANSLATIONS['Installer_Help'][self.lang]
                help_text = f"""
                <h3>{t_help['settings_help_title']}</h3>
                <p><b>{self.multi_thread_check.text()}:</b><br>{t_help['multi_thread_help']}</p>
                <hr>
                <p><b>{self.autoselect_check.text()}:</b><br>{t_help['auto_select_help']}</p>
                <p><b>{self.minimize_check.text()}:</b><br>{t_help['auto_minimize_help']}</p>
                <p><b>{self.detailed_categories_check.text()}:</b><br>{t_help['detailed_categories_help']}</p>
                <hr>
                <p><b>{self.notifications_label.text()}</b><br>{t_help['notifications_help']}</p>
                """
                self.help_dialog = HelpDialog(help_text, self)
            self.help_dialog.show()
            QTimer.singleShot(1, self.position_help_dialog)
        else:
            if self.help_dialog:
    ...
    def position_help_dialog(self):
        if self.help_dialog and self.help_dialog.isVisible():
            parent_geo = self.geometry()
            self.help_dialog.move(parent_geo.x() - self.help_dialog.width() - 10, parent_geo.y())
...
class AddSoftwareDialog(QDialog):
    def __init__(self, parent=None, lang='EN', settings=None):
        ...
        self.source_type_combo.addItems(["Unknown", "Package", "Link", "Winget"])
        ...
        cancel_button = buttons.button(QDialogButtonBox.StandardButton.Cancel)
        ok_button.setObjectName("acceptButton")
        ok_button.setMinimumSize(90, 30)
        cancel_button.setObjectName("cancelButton")
        cancel_button.setMinimumSize(90, 30)
        buttons.accepted.connect(self.accept)
        buttons.rejected.connect(self.reject)
        layout.addWidget(buttons)
    ...
    def get_data(self):
        ...
        if source_type != "Unknown" and not source_value:
