...
import sys, json, time, os, subprocess, tempfile, traceback, ctypes, re, winreg
...
class InstallerWorker(QObject):
    ...
    def run(self):
        if is_app_installed(self.name):
            self.progress.emit(self.name, "installed", {})
...
class InstallerWindow(QMainWindow):
    def __init__(self, app_instance, launcher_instance, language, theme, installer_data_file, download_repo, always_on_top):
        super().__init__()
        self.settings = {}
    ...
    def _init_ui_elements(self):
        ...
        self.btn_add_item = QPushButton("+", objectName="addButton", clicked=self.add_new_item)
        self.btn_remove_item = QPushButton("-", objectName="removeButton", clicked=self.remove_selected_items)
    ...
    def _hide_to_tray(self):
        self.hide()
        if self.settings.get("notifications", {}).get("on_hide", True):
    ...
    def load_installer_data(self):
        self.settings = {
            "show_progress": True,
            "auto_select_add": False,
            "auto_minimize_tray": True,
            "use_detailed_categories": False,
            "multi_thread_install": False,
            "notifications": {
                "enabled": True,
                "on_hide": True,
                "on_complete": True
    ...
    def _do_install(self):
        ...
        self.selected_items = list({item['name']: item for item in visible_items if item.get('source', {}).get('value') or item.get('source', {}).get('type') == 'Winget'}.values())
        ...
        if self.settings.get("show_progress", True):
            ...
            self.progress_dialog = ProgressDialog(self.total_items_to_install, self, lang=lang, always_on_top=self.always_on_top)
        ...
        if self.settings.get("multi_thread_install", False):
            max_workers = 5
        else:
            max_workers = 1
    ...
    def update_progress_ui(self, item_name, status, details):
        if self.settings.get("show_progress", True) and hasattr(self, 'progress_dialog'):
            if status == "failed" and self.retry_counts.get(item_name, 0) < 3:
                return 
    ...
    def on_task_finished(self, item_name, success):
        ...
        if success:
            ...
            if not self.settings.get("show_progress", True):
            ...
            if self.retry_counts[item_name] <= 3:
                self.update_progress_ui(item_name, "retrying", {"attempt": self.retry_counts[item_name]})
    ...
    def _finish_all_installations(self):
        ...
        if self.settings.get("show_progress", True) and hasattr(self, 'progress_dialog'):
            ...
            self.show_window()
        if self.settings.get("notifications", {}).get("on_complete", True):
