        ...
        let child_res = if app_type == "uwp" {
            ...
            if silent && !quiet_uninstall_string.is_empty() {
                ...
                if u_lower.contains("msiexec") {
                    ...
                    if silent {
                        let base_cmd_clean = base_cmd.replace("\"", "");
                        final_cmd = format!("\"{}\" /S /silent /verysilent /quiet /norestart", base_cmd_clean);
                    } else {
                        final_cmd = uninstall_string;
                    }
                }
            ...
            if is_missing {
                ...
                let ps_script = format!(
                    "$proc = Start-Process cmd.exe -ArgumentList '/c', '{}' -Wait -WindowStyle Hidden -PassThru -Verb RunAs; if ($proc) {{ exit $proc.ExitCode }} else {{ exit 1 }}",
                    final_cmd.replace("'", "''")
                );
                let mut cmd = Command::new("powershell");
        ...
        if is_missing {
            ...
            loop {
                ...
                if let Ok(Some(status)) = child.try_wait() {
                    success = status.success() || status.code() == Some(-1978334975) || status.code() == Some(3010) || status.code() == Some(1641);
